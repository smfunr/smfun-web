...
def log(event: str, **kw):
    print(json.dumps({"ts": datetime.now(timezone.utc).isoformat(), "event": event, **kw}, ensure_ascii=False), flush=True)
...
class Bot:
    def __init__(self):
        ...
        self.poll_ms = envi("POLL_INTERVAL_MS", 5000)
        self.scan_pages = envi("DISCOVERY_SCAN_PAGES", 8)
        ...
        log("BOT_START", dryRun=self.dry_run, chainId=self.chain_id, maxOrder=self.max_order_size)
    ...
    def tick(self):
        ...
        log("TICK", inWindow=in_entry_window, question=question, upAsk=up_ask, downAsk=down_ask, cap=round(self.capital, 2), holding=(self.pos.side if self.pos else None))

        if self.pos is None and in_entry_window:
            cands = []
            if up_ask <= self.entry_threshold:
                cands.append(("UP", up_tid, up_ask))
            if down_ask <= self.entry_threshold:
    ...
    def best_prices(self, token_id: str) -> Optional[Tuple[float, float]]:
    ...
    def close_pos(self, bid_price: float):
        ...
        log("CLOSED", side=p.side, entry=p.entry_price, exit=bid_price, pnl=round(pnl, 4), pnlPct=round((pnl / p.size_usdc) * 100, 2), capital=round(self.capital, 2))
        self.pos = None
